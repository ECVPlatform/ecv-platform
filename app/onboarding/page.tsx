      ...
      {step === 4 && (
        <div className="flex flex-col flex-1 bg-ink">
          ...
          <button
            ...
            className="flex items-center gap-1.5 font-sans font-light text-[10px] text-parchment/28 px-6 pt-2 bg-none border-none cursor-pointer a1"
          ...
          <div className="flex-1 px-6 overflow-y-auto no-scrollbar" style={{ paddingTop: '14px' }}>
            <p className="font-sans font-light text-[9px] text-parchment/20 a1" style={{ letterSpacing: '.25em', textTransform: 'uppercase' }}>
              Experience Creates Value
            </p>
            <h2 className="font-serif font-normal text-3xl text-parchment mt-3.5 a2" style={{ lineHeight: '1.22' }}>
              Not a marketplace.<br />Not a <em className="italic text-parchment/44">social</em> network.<br />Something that didn't exist before.
            </h2>
            <div className="w-6 my-4 a3" style={{ height: '1px', background: 'rgba(240,237,232,.1)' }} />
            ...
            <div className="a3">
              {[
                ...
                <div key={row.n} className="flex gap-3.5 py-3" style={{ borderBottom: '0.5px solid rgba(240,237,232,.07)' }}>
                  <div className="font-serif font-light text-[11px] text-parchment/14 w-4 flex-shrink-0 pt-0.5">{row.n}</div>
                  <div>
                    <div className="font-serif font-normal text-[16px] text-parchment/76">{row.t}</div>
                    <div className="font-sans font-light text-[11px] text-parchment/30 mt-0.5" style={{ lineHeight: '1.7' }}>{row.d}</div>
                  </div>
                </div>
              ))}
            </div>

            <div className="a4 mt-4 p-4 rounded-xl" style={{ border: '0.5px solid rgba(240,237,232,.08)' }}>
              <p className="font-sans font-light text-[8px] text-parchment/18 mb-2" style={{ letterSpacing: '.18em', textTransform: 'uppercase' }}>
              ...
              {['No likes. No followers. No feed.', 'Not everything meaningful needs an audience.', 'What you choose to keep is as valuable as what you share.', 'How much you reveal becomes part of who you are.'].map(line => (
                <p key={line} className="font-sans font-light text-[11px] text-parchment/27 py-0.5" style={{ lineHeight: '1.7' }}>{line}</p>
              ))}
            </div>

            <div className="a5 mt-3.5 mb-2 rounded-xl overflow-hidden" style={{ background: 'rgba(240,237,232,.05)', border: '0.5px solid rgba(240,237,232,.08)' }}>
              <div className="px-4 py-4">
                <p className="font-sans font-light text-[8px] text-parchment/18 mb-2" style={{ letterSpacing: '.2em', textTransform: 'uppercase' }}>
                  A Memoir — after the experience
                </p>
                <p className="font-serif font-normal text-[18px] text-parchment/60" style={{ lineHeight: '1.35' }}>
                  "The morning the city felt very far away."
                </p>
                <p className="font-sans font-light text-[11px] text-parchment/26 mt-2" style={{ lineHeight: '1.65' }}>
                  Before: to feel the river before Tokyo woke up.<br />After: more silence than I expected. Still thinking about it.
                </p>
                <div className="flex justify-between items-center mt-3 pt-2.5" style={{ borderTop: '0.5px solid rgba(240,237,232,.07)' }}>
                  <span className="font-sans font-light text-[9px] text-parchment/18">Akigawa Valley · Hiroyuki Oka · Oct 2025</span>
                  <span className="font-sans font-light text-[8px] text-parchment/16 px-2 py-0.5 rounded-full" style={{ letterSpacing: '.12em', textTransform: 'uppercase', border: '0.5px solid rgba(240,237,232,.09)' }}>
                    Private
                  </span>
                </div>
              </div>
